...
from data.ingestion.mt5_collector import MT5Collector, MT5Connector  # noqa: E402
...
class TestMT5CollectorInit:
    ...
    def test_default_output_dir(self, tmp_path, monkeypatch):
        ...
        collector = MT5Collector()
        assert collector.output_dir == tmp_path / "datasets" / "prices" / "raw"
...
class TestHealthCheck:
    @patch("data.ingestion.mt5_collector.MT5Connector")
...
# ---------------------------------------------------------------------------
# MT5Collector — _fetch_and_normalise (§3.1 compliance)
# ---------------------------------------------------------------------------


class TestFetchAndNormalise:
    """Focused unit tests for the §3.1 normalisation step."""
    ...
    @patch("data.ingestion.mt5_collector.MT5Connector")
    def test_time_converted_to_datetime(self, mock_cls, tmp_path):
        mock_inst = mock_cls.return_value
        mock_inst.fetch_ohlc.return_value = _MOCK_DF.copy()
