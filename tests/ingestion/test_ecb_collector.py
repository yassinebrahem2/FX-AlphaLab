"""Unit tests for ECB data collector (Bronze layer only).

Tests verify that ECBCollector:
- Fetches raw data from ECB SDMX API
- Adds source column
- Preserves all ECB source fields
- Does NOT perform any preprocessing
"""
...
class TestBaseCollector:
    ...
    def test_export_csv_writes_file(self, tmp_path):
        ...
        df = pd.DataFrame({"TIME_PERIOD": ["2023-01-01"], "OBS_VALUE": [1.05], "source": ["ecb"]})
...
class TestECBCollectorInit:
    def test_default_output_dir(self, tmp_path, monkeypatch):
        ...
        assert collector.output_dir == tmp_path / "raw" / "ecb"
...
# ---------------------------------------------------------------------------
# Bronze layer tests - NO preprocessing
# ---------------------------------------------------------------------------


class TestBronzeLayerContract:
    """Test that collector returns raw data with source column only."""

    def test_policy_rates_preserves_all_fields(self, tmp_path):
        collector = ECBCollector(output_dir=tmp_path)
        with patch.object(
        ...
        # All ECB fields must be preserved
        expected_ecb_fields = [
            "KEY",
            "FREQ",
            "REF_AREA",
        ...
        for field in expected_ecb_fields:
            assert field in result.columns, f"Missing ECB field: {field}"

        # Source column must be added
        assert "source" in result.columns
        assert (result["source"] == "ecb").all()

        # No transformation should occur
        assert len(result) == 2
        assert result.loc[0, "OBS_VALUE"] == "2.5"  # Still string, not float
        assert result.loc[0, "PROVIDER_FM_ID"] == "DFR"

    def test_exchange_rates_preserves_all_fields(self, tmp_path):
        collector = ECBCollector(output_dir=tmp_path)
        with patch.object(
            collector._session, "get", return_value=_make_response(SAMPLE_EXCHANGE_CSV)
        ):
        ...
        # No transformation should occur
        assert len(result) == 4
        assert result.loc[0, "OBS_VALUE"] == "1.0683"  # Still string
        assert result.loc[0, "CURRENCY"] == "USD"

    def test_no_preprocessing_columns_present(self, tmp_path):
        """Ensure NO Silver columns exist in Bronze output."""
        collector = ECBCollector(output_dir=tmp_path)
        with patch.object(
            collector._session, "get", return_value=_make_response(SAMPLE_POLICY_CSV)
        ):
        ...
        # These columns should NOT exist (they're Silver layer)
        silver_columns = ["timestamp_utc", "series_id", "value", "frequency", "units"]
        for col in silver_columns:
            assert col not in result.columns, f"Silver column '{col}' should not exist in Bronze"
    ...
    def test_collect_returns_both_datasets(self, tmp_path):
        ...
        with (
            patch.object(
                ...
                "get",
                side_effect=[
                    _make_response(SAMPLE_POLICY_CSV),
                    _make_response(SAMPLE_EXCHANGE_CSV),
                ],
            ),
            patch("time.sleep"),
        ):
        ...
        assert not result["policy_rates"].empty
        assert not result["exchange_rates"].empty
